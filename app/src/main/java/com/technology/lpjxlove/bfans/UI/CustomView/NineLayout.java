...
public class NineLayout extends ViewGroup {
    ...
    public NineLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a=context.obtainStyledAttributes(attrs, R.styleable.NineLayout,defStyleAttr,0);
        flag=a.getInteger(R.styleable.NineLayout_Flag,0);
        a.recycle();
    ...
    public void setAdapter(NineGridAdapter adapter, int flag) {
        ...
        if (flag==1){
            ...
            generateSendChildrenLayout(adapter.getCount());
        }
        //这里做一个重用view的处理
//        if (oldCount == 0) {
        removeAllViews();
...
//        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode==MeasureSpec.AT_MOST){
            totalWidth = 3*width/4 - getPaddingLeft() - getPaddingRight();
        }else {
        ...
        if (adapter != null && adapter.getCount() > 0) {
            ...
            if (childrenCount == 1) {
                if (flag==0){
                    ...
                    singleWidth=(totalWidth-gap*(4-1))/4;
                }
    ...
    private void generateSendChildrenLayout(int length){
        if (length<=4){
            rows=1;
            columns=length;
        }else if (length<=8){
